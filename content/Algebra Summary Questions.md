1. Consider $n=300$
	   1. Factorise $n$ (completely hence as a product of prime numbers).
	   2. Find all positive divisors of $n$ and arrange them in a Hasse diagram.
	   3. How many positive divisors does $n$ have?

2. Consider $k=75600$
	   1. Factorise $k$ into a product of prime factors.
	   2. Find the number of distinct positive divisors of $k$. (There is no need to compute them all in order to answer this question).
	   3. Find the number of distinct odd positive divisors of $k$.
	   4. Find the three largest positive divisors of $k$ (meaning largest in numerical value here so with respect to $\leq$ rather than $|$).
	    *Note that the Hasse diagram would be quite big in this case but it may still help you (or possibly not...) to draw just the top part of it.*

3. Use the Euclidean algorithm to find the greatest common divisor of 300 and 221.

4. Use the Euclidean algorithm to compute the greatest common divisor of 299 and 221. Now use the result to find the (complete) factorisations of 299 and 221.

5. Apply the Euclidean algorithm to find the greatest common divisor of $200000011 = 2 \times 10^8 + 11$ and $100000003 = 10^8 + 3$.

6. If $n$ is any positive integer show that $10n + 1$ and $2 \times 10n + 1$ are coprime.

7. Show that if the integer $a$ divides both integers $b$ and $c$ then $a$ divides each of $b + c$, $b - c$, and $bc$.

8. Use the definition of divisibility to show that to divide is a transitive relation which means the following: if $a | b$ and $b | c$ then $a | c$.

9. Consider the following statement: If $a, b \in \mathbb{Z}$ and $b | a$ then $b \leq a$.
	   1. Show that the statement is false (in the exact way it is written here).
	    *Hint: In order to prove that a statement is false it is enough to find a counterexample, that is one particular instance where it fails; in this case two particular numbers $a$ and $b$ for which it fails.*
	   2. Make a slight change to this false statement so it becomes a true statement.

10. Use the Euclidean algorithm to compute the greatest common divisor of 89 and 55.

11. Use the Euclidean algorithm to compute the greatest common divisor of 89 and 55, allowing negative remainders (with $-\frac{b}{2} < r \leq \frac{b}{2}$ instead of the usual $0 \leq r < b$).

12. Use the extended Euclidean algorithm to find integers $s$ and $t$ such that $89s + 55t = 1$.

13. Consider the two numbers $557$ and $337$.
	    1. Use the Euclidean algorithm to compute the greatest common divisor $d$ of 557 and 337.
	    2. Find integers $s$ and $t$ such that $557s + 337t = d$.

14. Find all integer solutions $x, y$ of $20x + 12y = 6$.

15. Consider the equation $19x + 12y = 1$.
	    1. Find all integer solutions $x, y$ of the equation.
	    2. Determine how many solutions satisfy $|x| < 12$ and $|y| < 19$.

16. Express the fraction $\frac{1}{77}$ as a difference $\frac{a}{b}$ for some positive integers $a, b$. Then express the same fraction $\frac{1}{77}$ as a difference $\frac{c}{d}$ for some different positive integers $c, d$.

17. In a fictional country with currency 'unit' and only two types of coins - a 'small coin' worth 13 units and a 'big coin' worth 17 units...
...
38. Use Ruffini’s rule to verify that $ix^4 + (i - 1)x^3 - (4 + i)x^2 - (2 + 3i)x - 6 + 3i$ is a multiple of $x + 2 - i$ and determine their quotient.
...
53. Compute the square roots of the complex number $\frac{5}{4} + 3i$ expressing them using only square roots of real numbers (that is expressing their real and complex part using only algebraic operations on real numbers).
...
66. Find integers for the following:
...
82. Simply by testing all possibilities for $x \in Z/nZ$ find all solutions of each of the following equations in $Z/nZ$ (where we have abbreviated $[a]_n$ to $[a]$ for simplicity):
	5. $x^2 = [-1]$ in $Z/3Z$;
	6. $x^2 = [-1]$ in $Z/5Z$;
	7. $x^2 = [-1]$ in $Z/7Z$;
	8. $x^2 = [-1]$ in $Z/11Z$;
